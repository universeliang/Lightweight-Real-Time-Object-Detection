...
class GhostModule(nn.Module):
    ...
    def __init__(self, num_in, num_out, kernel_size=1, stride=1, padding=0, ratio=2, dw_size=3,
        ...
        init_channels = math.ceil(num_out / ratio)  # 向上取整, 输出通道数除以ratio，缩小二倍
        new_channels = init_channels * (ratio - 1)  # 相当于没变
        ...
        # 3*3的dw卷积，group等于输入的通道（这里因为init_channels和 new_channels其实是一样的,group无所谓。但看这个意思，dw卷积的group，应该都是输入通道）。用的relu激活
    ...
    def forward(self, x):
        x1 = self.primary_conv(x)  # 先1*1
        x2 = self.cheap_operation(x1)  # 后3*3
        return torch.cat([x1, x2], dim=1) # 拼接
...
class GhostModuleMul(nn.Module):
    ...
    def __init__(self, num_in, num_out, kernel_size=1, stride=1, padding=0, ratio=2, dw_size=3,
        ...
        self.avgpool2d = nn.AvgPool2d(kernel_size=2, stride=2)  # 平均池化，这是干什么的呢？
        self.gate_fn = Activation('sigmoid')  # 调用Activation里面的sigmoid函数
        init_channels = math.ceil(num_out / ratio)  # 输出通道out除以一半
        ...
        self.cheap_operation = ConvUnit(init_channels, new_channels, kernel_size=dw_size, stride=1,
                                        padding=dw_size // 2, num_groups=init_channels,
                                        use_act=use_act, act_type=act_type)
        # 先1*1卷积，不用激活函数act；再接两个正交卷积，也不用激活函数
    ...
    def forward(self, x):
        res = self.avgpool2d(x)  # 先对 x 平均池化，然后进行short_conv操作，接着用sigmoid函数
        res = self.short_conv(res)
        res = self.gate_fn(res)

        x1 = self.primary_conv(x)  # 先对 x 进行1*1卷积，然后3*3的dw，再拼接
